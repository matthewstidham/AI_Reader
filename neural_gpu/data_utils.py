...
def get_batch(max_length, batch_size, do_train, task, offset=None, preset=None):
  ...
  for b in range(batch_size):
    ...
    inputs.append(inp + [0 for l in range(pad_length - len(inp))])
    targets.append(target + [0 for l in range(pad_length - len(target))])
  res_input = []
  res_target = []
  for l in range(pad_length):
    new_input = np.array([inputs[b][l] for b in range(batch_size)],
                         dtype=np.int32)
    new_target = np.array([targets[b][l] for b in range(batch_size)],
                          dtype=np.int32)
...
def accuracy(inpt, output, target, batch_size, nprint):
  ...
  def task_print(inp, output, target):
    ...
    print_out("    o: " +
              " ".join([str(output[l] - 1) for l in range(print_len)]))
    print_out("    t: " +
              " ".join([str(target[l] - 1) for l in range(print_len)]))
